...
for row in reader:
    ...
    for i in range(len(row) - j):
        ...
        if active_field.isdigit():
            ...
            fields[header_row[i + j]] = new_number
        else:
            if header_row[i + j] == "data":
                date_str = active_field.strip()  # The date - 29 Dec 2017
                format_str = '%d/%m/%y'  # The format
                datetime_obj = datetime.datetime.strptime(date_str, format_str)
                fields[header_row[i + j]] = str(datetime_obj.date())
            else:
                fields[header_row[i + j]] = active_field.strip()
